...
source "${tcf_package_script_dir}/url_0.1.sh"
source "${tcf_package_script_dir}/file-util_0.1.sh"
source "${tcf_package_script_dir}/array-util_0.1.sh"
...
# use zip for lambda layers
    ...
    debugLog "executing: zip -r \"${job_zip_target_dir}/${job_file_root}.zip\" * > /dev/null"
    zip -r "${job_zip_target_dir}/${job_file_root}.zip" ./* > /dev/null
    cd "${current_dir}"
